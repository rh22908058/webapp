  ...
  <div class="book-list">
    <div class="top">
      <div class="box-wrapper">
          <search-box ref="box" @query="onQueryChange" class="search-box" placeholder='搜索图书'>
          </search-box>
      </div>
      <div class="cancel" @click="back">取消</div>
    </div>
    <scroll ref="scroll" class="content" :data="books" :pullup="true" @scrollToEnd="searchMore">
      <ul>
        <li @click="selectItem(book,$event)" v-for="(book,index) in books" class="book" ref="book">
          <img v-lazy="book.image_small" class="image">
          <div class="info">
            <div class="line">
              <span class="name">{{book.name}}</span>
              <span class="average">评分:{{book.average}}</span>
            </div>
            <div class="line">
              <span class="director">作者:{{book.author}}.出版年份:{{book.pubdate}}</span>
            </div>
          </div>
        </li>
    ...
    <scroll ref="searchContent" class="content search-content" :data="searchList" :pullup="true" @scrollToEnd="searchMore" v-show="showFlag">
        <ul>
            <li v-for="(book,index) in searchList" class="book" @click.stop.prevent="selectItem(book,$event)">
                <img :src="book.image_small" class="image">
                <div class="info">
                  <div class="line">
                    <span class="name">{{book.name}}</span>
                    <span class="average">评分:{{book.average}}</span>
                  ...
                  <div class="line">
                    <span class="director">导演:{{book.author}}.年份:{{book.pubdate}}</span>
                  </div>
                </div>
            </li>
...
import {getBooks,getBook,searchBooks} from '../api/books'
...
export default {
  ...
  created(){
    this.loadMore=true
    getBooks(this.show_page).then(res=>{
      console.log(res)
      this.show_page+=10
      ...
      res.books.forEach((item)=>{
        this.books.push(createBook(item))
      })
      console.log(res)
      console.log('pic')
      console.log(this.books[1].image_small)
    })
    /*
  ...
  methods:{
    ...
    _searchBooks(){
      searchBooks(this.query,this.search_page).then(res=>{
        this.loadMore=true
        this.search_page+=10
        ...
        res.books.forEach((item)=>{
          this.searchList.push(createBook(item))
        })
      })
    }
  },
